...
def plot_and_save_all(t, solution, r_0, t_star):
    if not os.path.exists(PATH + "/plots"):
        ...
        os.makedirs(PATH + "/plots/controlled_models")
        os.makedirs(PATH + "/plots/final_size")
    plt.plot(t, solution[:, 1:], linewidth=2)
    plt.xlabel("Time (days)")
    plt.ylabel("State values")
    plt.legend(('E1(t)', 'E2(t)', 'I1(t)', 'I2(t)', 'I3(t)', 'R(t)'), loc='upper right')
    plt.savefig(PATH + "/plots/controlled_models/solution_SEIR_control_R0_" + str(r_0) + "_t_" + str(t_star) + "_all.png",
                format="png")
    plt.savefig(PATH + "/plots/controlled_models/solution_SEIR_control_R0_" + str(r_0) + "_t_" + str(t_star) + "_all.eps",
                format="eps")
    plt.close()
...
def plot_final_sizes(r_0, t_star, final_size):
    ...
    plt.savefig(PATH + "/plots/final_size/final_size_R0_" + str(r_0) + ".png",
                format="png")
    plt.savefig(PATH + "/plots/final_size/final_size_R0_" + str(r_0) + ".eps",
                format="eps")
    plt.close()
...
def plot_final_sizes_in_one(t_stars, final_sizes):
    ...
    plt.tight_layout()
    plt.savefig(PATH + "/plots/final_size/final_sizes_all.png",
                format="png", dpi=500)
    plt.savefig(PATH + "/plots/final_size/final_sizes_all.eps",
                format="eps", dpi=500)
    plt.close()
